		...
		<table bgcolor = "#CCCCCC" cellspacing = 1 cellpadding = 5 width = 100% border=2>
			...
			<tr bgcolor = "#FFFFFF">
				...
				<td align = "center" name = 'booktype'><%=book.getBook_type()%></td>
					<td align = "center"><a href='deleteBook.jsp?id=<%= book.getBook_id()%>&bookname=<%=book.getBook_name() %>&booknum=<%=book.getBook_num() %>&bookowner=<%=book.getBook_owner() %>&booksource=<%=book.getBook_source() %>&booktype=<%=book.getBook_type()%>&bookstatus=<%=book.getBook_status() %>' onclick='return confirm("确定删除该记录？")'>删除</a>
				<a href='updateBook.jsp?id=<%= book.getBook_id()%>&bookname=<%=book.getBook_name() %>&booknum=<%=book.getBook_num() %>&bookowner=<%=book.getBook_owner() %>&booksource=<%=book.getBook_source() %>&booktype=<%=book.getBook_type() %>&bookstatus=<%=book.getBook_status() %>'>修改</a>
			</td>
			</tr>
			<%
				}
