...
<body>
    ...
    <% 
			...
			BookDAO bookDAO = (BookDAO) factory.getBean("BookDAO");
			 List<Book> books = bookDAO.FindBookByUsername(new String(request.getParameter("username").toString().getBytes("ISO-8859-1"),"UTF-8"));
		...
		<table bgcolor = "#CCCCCC" cellspacing = 1 cellpadding = 5 width = 100% border=2>
			...
			<tr bgcolor = "#FFFFFF">
				...
				<td align = "center"><a href='returnUserBook.jsp?id=<%= book.getBook_id()%>&bookname=<%=book.getBook_name() %>&booknum=<%=book.getBook_num() %>&bookowner=<%=book.getBook_owner() %>&booksource=<%=book.getBook_source() %>&booktype=<%=book.getBook_type()%>&bookstatus=<%=book.getBook_status() %>' onclick='return confirm("确定归还该图书？")'>归还图书</a>
				</td>
			</tr>
			<%
				}
