...
<body>
    ...
    <div style="float:left"><h2>&nbsp;<img src="images/hotel.png" width="32" width="68" />&nbsp;图&nbsp;书&nbsp;在&nbsp;库&nbsp;信&nbsp;息：</h2></div>
     ...
     <div style="float:left">
       ...
       <form action = "showFindUserBook.jsp" method = "post"> 
    ...
    <% 
		...
		<table bgcolor = "#CCCCCC" cellspacing = 1 cellpadding = 5 width = 100% border=2>
			...
			%>
			<tr bgcolor = "#FFFFFF">
				<!--<td><input type = 'checkbox' name = 'id' value = "<%=rs.getInt("book_id")%>"/></td>-->
				...
				<td align = "center"><a href='borrowBook.jsp?id=<%= rs.getInt("book_id")%>&bookname=<%=rs.getString("book_name")%>&booknum=<%=rs.getString("book_num")%>&bookowner=<%=rs.getString("book_owner")%>&booksource=<%=rs.getString("book_source")%>&bookstatus=<%=rs.getString("book_status")%>&booktype=<%=rs.getString("book_type")%>' onclick='return confirm("确定借阅该图书？")'>借阅图书</a>
				</td>
			</tr>
			<%
				}
